...
export default async function handler(req, res) {
    ...
    for (let i = 0; i < required_inputs.length; i++) {
        ...
        if (k == "delay") {
            if (
                ![10, 30, 60, 120, 360, 720, 1440].includes(parseInt(inputs[k]))
            ) {
                wrong = true;
            }
        }
    ...
    if (inputs["amount"]) {
        ...
        if (inputs["output_network"] != "visa") {
            contract_amount = [50000000000000000000, 100000000000000000000];
        }
        ...
        if (inputs["input_network"] == "doge") {
            contract_amount = [
                500000000000000000000, 1000000000000000000000,
                10000000000000000000000, 50000000000000000000000,
            ];
        }
            ...
            res.status(200).json({
                ...
                message: {
                    ...
                    input: {
                        ...
                        plan: response.content.plan ?? 0,
                        amount:
                            response.content.amount ??
                            inputs["amount"] ??
                            false,
                    ...
                    tracking: response.content.tracking,
                },
            });
        })
        .catch((error) => {
